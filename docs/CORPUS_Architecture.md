...
| **Block 2** | Caliber (migración desde ADS + alcance nuevo) + Cortex (táctica + afecto NPC) | **Caliber: CERRADO — verificado en juego el 2026-07-09** (primer consumidor real de las 6 primitivas; su boot diferido a `Initialize` es hoy el patrón template del ecosistema, §6; doc particular: `corpus-caliber/docs/Caliber_Architecture.md`. El alcance nuevo —armadura de jugador— es su **Block 3**, EN CURSO: su tramo 0 cerró el 2026-08-23 con el **contrato de datos Cargo ↔ Caliber** documentado en `Caliber_Architecture.md` **§13** —CAL-24/25/26 y CRG-77/78— y con el reparto de `ply:Armor()` **medido en juego** y no citado del engine: un punto de armadura absorbe exactamente un punto de daño, `DMG_FALL` no toca el pool, y con eso `ply:Armor()` y los puntos de escudo quedan en la misma unidad. Falta el paso 2, que es la implementación. Ver §7) · **Cortex: EN DISEÑO** — el repo se fundó el 2026-08-24 (CLAUDE.md + estado + roadmap + `Cortex_Architecture.md`; familia `CTX` con sus cinco primeras normas) y abrió su bloque de **escuadrones**, con alcance votado: una CAPA sobre NPCs de VJ Base y HL2, no una base de NPC nueva (**CTX-1**). Sin código todavía. ⚠ El gate de los eventos daño/limb cubre **el afecto y no el módulo entero** (**CTX-2**), y es lo que permitió abrir el bloque con el Block 3 de Caliber en su tramo 0. Doc particular: `corpus-cortex/docs/Cortex_Architecture.md` |
| **Block 3** | Coagulant (médico jugador, estilo ACE3 — heridas por zona, sangrado, vitales, tratamiento, debuffs zonales y UI) | **Cerrado (2026-07-20).** Los 4 slices verificados en juego (rondas 1-7; la 7 pasó 13/13 e incluyó el modo degradado sin Cargo) y los fixes post-cierre confirmados (mini-ronda 8 y check N1). Restan dos decisiones de diseño abiertas; el tramo de zonas `chest`/`stomach` está completo (COA-8/COA-7, 2026-07-21: ratificado, bajado a código y verificado en juego — ronda O, 6/6). Doc particular: `corpus-coagulant/docs/Coagulant_Architecture.md`; foto en su `coagulant_estado.md` |
