...
10. **COR-17 — Los assets de terceros no se versionan.** El banco de sonidos default del ecosistema vive en este repo (`sound/corpus/`, ports de STALKER GAMMA ordenados por consumidor — decisión del autor 2026-07-24) pero **fuera de git**: la MIT cubre código y docs, nunca assets ajenos — mismo régimen que **STK-2** en `corpus-stalker`. Los `about.txt` del árbol **sí** se versionan (documentación propia: mapa del banco, renombres, consumidores — el índice vive en `sound/corpus/about.txt`). Hospedar el banco **no** viola COR-10: es infraestructura de assets compartida por los módulos, sin una línea de lógica; los módulos lo consumen **por detección** (`file.Exists`, COR-5), jamás lo asumen montado.
